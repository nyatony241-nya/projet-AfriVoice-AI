...
const StatCounters: React.FC<StatCountersProps> = ({
  ...
  const stats = [
    ...
    },
    {
      ...
      sub: 'Aoede, Kore, Puck, Charon, Fenrir',
      icon: (
        <svg className="w-4 h-4 sm:w-5 sm:h-5 text-black" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    },
    {
      ...
      sub: isEn ? 'Professional Audio Studio' : 'Studio Audio Professionnel',
      icon: (
        <svg className="w-4 h-4 sm:w-5 sm:h-5 text-black" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div
          ...
          className={`p-2 sm:p-2.5 rounded-xl border transition-all duration-300 hover:-translate-y-0.5 group relative overflow-hidden flex flex-col justify-between min-w-0 ${
            isDark
              ...
              : 'bg-white border-[#E4E4E7] hover:border-[#D4FF00]/40 shadow-xs'
          }`}
          style={{ animationDelay: `${i * 100}ms` }}
        >
          {/* Subtle gradient glow inside */}
          <div className="absolute top-0 right-0 -mr-6 -mt-6 w-16 h-16 rounded-full bg-gradient-to-br from-[#D4FF00]/10 dark:from-[#D4FF00]/10 to-transparent blur-xl pointer-events-none group-hover:scale-125 transition-transform duration-500" />
...
export default React.memo(StatCounters);
